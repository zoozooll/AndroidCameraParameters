...
![應用概覽](../../website/static/img/camera_params_feature_graph.png)

## 核心特性

*   **詳細診斷**：檢查所有鏡頭（後置、前置、外置）的 `CameraCharacteristics`。
*   **硬體層級檢測**：即時查看您的設備是否支援 `LEGACY`、`LIMITED`、`FULL` 或 `LEVEL_3` 特性。
*   **即時特性跟蹤**：透過直觀的儀表板檢查對 RAW 擷取、光學防手震 (OIS)、手動曝光、手動對焦等功能的支援。
*   **分類探索**：數百個參數按感測器、鏡頭、AE/AF/AWB 和處理類別組織，並具有**內建搜尋**功能。
*   **收藏夾（即將推出）**：書籤常用參數以便快速存取。
